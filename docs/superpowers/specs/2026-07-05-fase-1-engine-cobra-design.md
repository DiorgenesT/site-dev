...
**Tabela de offsets do traco multi-passada:** gerada uma unica vez no boot (funcao pura, deterministica, sem `Math.random`), com 2 a 3 "passadas" por segmento do corpo, cada uma com um pequeno deslocamento fixo e espessura variavel pre-calculada. Estilo escolhido no brainstorming visual: traco multi-passada (rabisco nervoso, duas ou tres passadas levemente desalinhadas sobrepostas), mesmo principio das tabelas de rotacao/jitter dos tokens da Fase 0 (nunca ruido calculado por frame).

---

## 3. Docking (zona de proximidade, scrub, reversivel)

`docking.ts` expoe uma funcao pura `fatorDocking(progresso: number, inicioZona: number): number` que retorna 0 antes da zona, sobe suavemente (curva de easing tipo smoothstep) conforme o progresso avanca dentro da zona (ex: ultimos 8% antes do waypoint final), e chega a 1 exatamente no waypoint. Por ser uma funcao pura do progresso atual (sem estado de "ja animou"), a reversao e automatica: se o scroll volta, o fator cai junto, sem logica extra de "desfazer".
